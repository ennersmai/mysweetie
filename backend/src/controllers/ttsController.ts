...
export const handleArcanaPcm = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // Daily reset for free trials using Redis fallback (no schema change required)
    if (!isPremium && redis.isConnected()) {
      const client = redis.getClient();
      const resetKey = `tts:free_reset:${userId}:${todayUtc()}`;
      try {
        ...
        if (!wasReset) {
          ...
          await client?.setEx(resetKey, 60 * 60 * 24, '1');
        }
      } catch (e) {
